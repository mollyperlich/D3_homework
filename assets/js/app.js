...
// read the data
    d3.csv("/data/data.csv", function(err, brfss) {
    if (err) throw err;
    console.log(brfss)
    
    // process the data
    for (var i = 0; i < brfss.length; i++) {
        console.log(i, brfss[i].state, brfss[i].poverty, brfss[i].healthcare );
        console.log(i, brfss[i].obesity, brfss[i].income  );
    }

    brfss.forEach(function(data) {
